<template>
  <div class="login-container">
    <el-form ref="form" :model="form" :rules="rules" class="login-form" label-position="left">
      ...
      <h2 align="center">{{ $t('change_password') }}</h2>
      <p align="center">{{ $t('text_password') }}</p>
      ...
      <el-tooltip v-model="capsTooltip" :content="this.$t('content_caps_lock')" placement="right" manual>
        <el-form-item prop="password_old" :error="error_current">
          ...
          <span class="show-pwd" @click="showPwd('current')">
            <svg-icon :icon-class="passwordTypeCurrent === 'password' ? 'eye' : 'eye-open'" />
          </span>
        </el-form-item>
      </el-tooltip>
      <el-tooltip v-model="capsTooltip" :content="this.$t('content_caps_lock')" placement="right" manual>
      ...
      <el-tooltip v-model="capsTooltip" :content="this.$t('content_caps_lock')" placement="right" manual>
        <el-form-item prop="password_match" :error="error_not_match">
          ...
          <el-input
            ...
            @keyup.native="checkCapslock"
          />
          <span class="show-pwd" @click="showPwd('renew')">
...
import { allCharacterExcepJapanese } from '@/utils/validate'
    ...
    return {
      rules: {
        ...
        password: [
          ...
          { max: 16, message: this.$t('max_length') + '16 ' + this.$t('characters'), trigger: 'blur' },
          { validator: allCharacterExcepJapanese, trigger: 'blur' }
        ],
